...
## 实验基本信息
- **实验名称**：五级流水线 CPU 的运算器、缓存与分支预测器实现
- **使用语言**：Verilog HDL
- **仿真工具**：Verilator + Difftest
- **目标架构**：RISC-V 32I + RV32M (乘除法)
- **流水线级数**：5 级 (IF → ID → EX → MEM → WB)
- **实验周期**：3 个 Task，递进式设计

## 一、 实验目的与要求
1. **理解并实现底层运算器模块**：在不依赖 IP 核或高级运算符的前提下，完成高性能乘法器（华莱士树）和多周期除法器（Radix-2 状态机）的底层设计并接入流水线。
2. **掌握流水线与多周期存储的握手机制**：将实验五完成的 Cache 模块接入五级流水线，处理 Cache Miss 带来的流水线动态阻塞及相关的复杂数据冒险。
3. **深入理解分支预测技术**：实现全局分支预测器与局部分支预测器，并设计竞争选择器（Meta Predictor）构建混合自适应分支预测架构，将其成功接入流水线以减少控制冒险带来的性能损失。

## 二、 实验内容与核心原理

### 1. Task 1: 乘法器与除法器的建立及流水线接入

#### 1.1 乘法器（基于 CSA 的单周期华莱士树）
...
### 2. Task 3: Cache 成功接入流水线

**实现背景**：将单周期的 Data Memory 替换为多级缓存，需要处理 Cache Miss 导致的多周期延迟、复杂的流水线停顿与数据冒险、以及 Difftest 验证框架的同步问题。本 Task 中 Cache 采用 FIFO/LRU/Rand 多种替换策略实现，命中延迟约 2-4 周期，Miss 延迟最多 10-20 周期。
