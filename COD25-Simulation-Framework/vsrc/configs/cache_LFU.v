...
module cache #(
    ...
    // LFU替换策略：每个 Cache Line 维护访问次数，替换访问次数最少的路。
    // 若当前组存在无效块，仍然优先填入无效块。
    reg [7:0] freq [0:SET_NUM-1][0:WAY_NUM-1];
    ...
    // 找出该换哪一路：Invalid 优先，否则选择访问次数最少的路。
    integer w;
    reg [7:0] min_freq;
    reg found_invalid;
    always @(*) begin
        replace_way = 0;
        min_freq = freq[w_index][0];
        found_invalid = 1'b0;
        for (w=0; w<WAY_NUM; w=w+1) begin
            if (!valid[w]) begin
                replace_way = w;
                found_invalid = 1'b1;
            end else if (!found_invalid && freq[w_index][w] <= min_freq) begin
                min_freq = freq[w_index][w];
                replace_way = w;
            end
        end
    end

    // 命中时增加访问次数；refill 后将新块访问次数置为 1。
    integer i, j;
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            if ((CS == READ || CS == WRITE) && hit && miss == 1'b0) begin
                if (freq[w_index][hit_way] != 8'hff) begin
                    freq[w_index][hit_way] <= freq[w_index][hit_way] + 1'b1;
                end
            end else if (CS == IDLE && refill) begin
                freq[w_index][replace_way_buf] <= 8'd1;
            end
        end
    end
