...
module cache #(
    ...
    //LRU替换策略
    // 为每个 Set 的每个 Way 维护一个 Age 计数器
    reg [WAY_INDEX_WIDTH-1:0] age [0:SET_NUM-1][0:WAY_NUM-1];
    ...
    // 找出该换哪一路（挑选当前Index下Age处于最大值的Way，或者是无效的Way）
    integer w;
    always @(*) begin
        replace_way = 0;
    ...
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            // 发生 Cache Hit
            if ((CS == READ || CS == WRITE) && hit && miss == 1'b0) begin
                for (j=0; j<WAY_NUM; j=j+1) begin
                    if (age[w_index][j] < age[w_index][hit_way]) begin
                        age[w_index][j] <= age[w_index][j] + 1;
                    end
                end
                age[w_index][hit_way] <= 0; // 将刚刚使用的设为0
            end 
            // 发生 Cache Miss 且重新填充后（相当于新数据也刚被访问）
            else if (CS == IDLE && refill) begin
                for (j=0; j<WAY_NUM; j=j+1) begin
                    if (age[w_index][j] < age[w_index][replace_way_buf]) begin
                        age[w_index][j] <= age[w_index][j] + 1;
                    end
                end
                age[w_index][replace_way_buf] <= 0;
            end
        end
    end
