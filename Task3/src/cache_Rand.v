...
module cache #(
    ...
    // 伪随机替换策略：使用一个循环计数器近似随机选择替换路。
    // 若当前组存在无效块，仍然优先填入无效块。
    reg [WAY_INDEX_WIDTH-1:0] rand_counter;
    ...
    // 找出该换哪一路：Invalid 优先，否则使用循环计数器给出的伪随机路号。
    integer w;
    always @(*) begin
        replace_way = rand_counter;
    ...
    // 每完成一次 refill 后推进计数器，使下次替换选择不同路。
